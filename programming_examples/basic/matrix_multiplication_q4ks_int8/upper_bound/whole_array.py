...
def _build_design(
    ...
    *,
    ...
    fifo_depth = 2
    # INT32 output tiles are four times larger than INT8 operands. Preserve
    # depth two when it fits because it materially improves overlap; use one
    # C object only for wider experimental tiles that otherwise exceed 64 KiB.
    c_l1_depth = fifo_depth
    if dtype_out is np.int32:
        fixed_core_bytes = 2 * m * k + 2 * k * n + 0xD00 + 4 * 1024
        if fixed_core_bytes + fifo_depth * m * n * 4 > 64 * 1024:
            c_l1_depth = 1
        assert fixed_core_bytes + c_l1_depth * m * n * 4 <= 64 * 1024
        ...
        for _ in loop:
            ...
            for _ in range_(K // k):
                ...
                if kernel_repeats == 1:
                    ...
                    # Hold one A/B tile resident and repeatedly execute the
                    # complete zero+MMUL kernel. The final value is the last
                    # K tile's dot product; all repeats count toward the raw
                    # resident compute ceiling.
                    for _ in range_(kernel_repeats):
                        zero(elem_out)
                        matmul(elem_in_a, elem_in_b, elem_out)
                in_a.release(1)
                in_b.release(1)
...
def whole_array(
    ...
    *,
    ...
    kernel_repeats: CompileTime[int] = 1,
):
...
def _make_argparser():
    ...
    p.add_argument("--kernel-repeats", type=int, default=1)
...
def _validate_shape_args(opts):
    ...
    if opts.kernel_repeats < 1:
        sys.exit("--kernel-repeats must be positive")
...
def _compile_kwargs(opts):
    return dict(
        ...
        kernel_repeats=opts.kernel_repeats,
    )
...
def _run_and_verify(opts):
    ...
    if opts.kernel_repeats == 1:
        expected_logical = _numpy_reference(
            A_np, B_np, opts.b_col_maj, dtype_out
        )
    else:
        B_logical = B_np.T if opts.b_col_maj else B_np
        expected_logical = (
            A_np[:, -opts.k :].astype(np.int64)
            @ B_logical[-opts.k :, :].astype(np.int64)
        ).astype(dtype_out)
    if opts.c_col_maj:
    ...
    assert_close_with_benchmark(
        ...
        expected,
        bench=bench,
        ops=2.0 * opts.M * opts.K * opts.N * opts.kernel_repeats,
